...
function BreakTaskModal({
    ...
    useEffect(() => {
        ...
        fetchStatus();
    }, [isOpen, student, blockId, routines, missions]); // deps에 routines, missions 추가
        ...
        "
            >
                ...
                <h2 className="text-xl font-bold mb-2 flex items-center justify-between">
                    <span>🎯 {student.name} 학생 쉬는시간 도전</span>
                    ...
                    <button
                        ...
                        onClick={async () => {
                            ...
                            try {
                                ...
                                // -----------------------------
                                if (routines.length > 0) {
                                    // 오늘 + 해당 블록의 이 학생 루틴 상태 삭제
                                    const { error: deleteError } = await supabase
                                        .from("student_break_routine_status")
                                        .delete()
                                        .eq("student_id", student.id)
                                        .eq("date", today)
                                        .eq("block_id", blockId); // block_id 필수
                                    ...
                                    const routineInserts = Object.entries(routineStatus).map(
                                        ([rid, completed]) => ({
                                            ...
                                            routine_id: rid, // routine_item_id -> routine_id 로 변경
                                            completed,
                                            date: today,
                                            block_id: blockId, // block_id 삽입 필수
                                        })
                                    );
